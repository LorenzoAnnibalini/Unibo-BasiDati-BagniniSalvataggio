<?php
    ...
    if(isset($_POST['elimina'])){
        ...
        $sql = "DELETE FROM `POSSIEDE` WHERE `IdAttrezzatura`='".$id."' AND `Nome`='".$nome."'";
        $result = $db->query($sql);
...
?>
    ...
    <head>
        ...
        <title>Elenco Materiale Torretta</title>
    ...
    <body class="text-center">
        <div class="container">
            ...
            echo "<h1 class='mt-5'>Elenco Materiale Torretta ".$id."</h1>"; 

            // Query per ottenere i dati dei dipendenti
            $sql = "SELECT `Nome`,`IdAttrezzatura` FROM `POSSIEDE` WHERE `IdTorretta`='".$id."';"; // Cambia 'dipendenti' con il tuo nome di tabella
            $result = $db->query($sql);
            ...
            if ($result->num_rows > 0) {
                ...
                while($row = $result->fetch_assoc()) {
                    echo "<tr><td>" . $row["Nome"] . "</td><td>" . $row["IdAttrezzatura"] ."</td><td><form action='' method='POST'><input type='hidden' name='id_attrezzatura' value='".$row['IdAttrezzatura']."'><input type='hidden' name='id_torretta' value='".$id."'><input type='hidden' name='nome_attrezzatura' value='".$row['Nome']."'><button class='w-100 btn btn-lg btn-outline-danger' type='submit' name='elimina'><i class='bi bi-trash'></i></button></form></td></tr>";
                }
                echo"<tr><td><form action='assegnaattrezzaturatorretta.php' method='POST'><button class='w-100 btn btn-lg btn-primary' type='submit' name='IdTorretta' value='".$id."'><i class='bi bi-hammer' style='color: white'></i><h4>Aggiungi</h4></button></form></td></tr>";
